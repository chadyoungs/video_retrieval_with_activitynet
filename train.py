...
def get_video_file_demo_list():
    ...
    file_list = [os.path.join(root, file) for file in os.listdir(root) if file.endswith("avi")]
...
def train():  
    ...
    for idx, video_file in enumerate(video_file_list):
        video_embedding = generate_video_embedding(video_file, FRAME_SAMPLING_RATE, EMBEDDING_DIM)
        
        if video_embedding:
            # Prepare data for insertion (Milvus requires columns)
            data = [
                {"video_id": idx, "video_filepath": video_file, "clip_vector": video_embedding}
            ]
