...
def get_text_embedding(query_text: str) -> List[float]:
    ...
    with torch.no_grad():
        text_features = get_text_features(_model, text_inputs)
...
def get_image_embedding(query_img_path: str) -> List[float]:
    ...
    with torch.no_grad():
        img_features = get_image_features(_model, img_inputs)
...
def hybrid_retrieval(
    ...
    if annotation_conditions and not sql_hits:
        print("SQL search returned empty results for conditions:", annotation_conditions)
...
if __name__ == "__main__":
    ...
    if option == "text":
        query_text = "busy city driving with cars and pedestrians"
        annotation_conditions = {"scene_env": "outdoor", "scene_type": "street"}
        metadata_filters = {"dataset_name": "nuscenes", "camera_channel": "CAM_FRONT"}
        retrieval_with_text(
            query_text,
            annotation_conditions=annotation_conditions,
            metadata_filters=metadata_filters,
